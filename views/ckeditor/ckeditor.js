...
(function() {
 ...
 (function() {
  ...
  CKEDITOR.tools.extend(CKEDITOR.dom.element.prototype, {
   ...
   removeClass: e ? function(a) {
    ...
    c && a(c, b) && ((c = c.replace(new RegExp("(?:^|\\s+)" + b + "(?=\\s|$)"), "").replace(/^\s+/, "")) ? this.setAttribute("class",
 ...
 (function() {
  ...
  CKEDITOR.dom.range.prototype = {
   ...
   createBookmark: function(a) {
    ...
    p || (c = b.clone(), c.setHtml("&nbsp;"), a && c.setAttribute("id", d + "E"), e = this.clone(),
 ...
 (function() {
  CKEDITOR.inline = function(a, d) {
   ...
   c ? (b.setData(c.getValue(), null, !0), a = CKEDITOR.dom.element.createFromHtml('<div contenteditable="' + !!b.readOnly + '" class="cke_textarea_inline">' + c.getValue() + "</div>", CKEDITOR.document),
    ...
    if (a && a.isInline()) {
     ...
     if (d && (c = this.ui.space(this.elementMode == CKEDITOR.ELEMENT_MODE_INLINE ? "top" : "contents"))) {
      var e = CKEDITOR.tools.getNextId(),
       d = CKEDITOR.dom.element.createFromHtml('<span id="' + e + '" class="cke_voice_label">' +
        d + "</span>");
      c.append(d);
    ...
    labeledElement: function(b, a, d, f) {
     if (!(4 > arguments.length)) {
      ...
      CKEDITOR.ui.dialog.uiElement.call(this, b, a, d, "div", null, e, function() {
       ...
       "horizontal" != a.labelLayout ? e.push('<label class="cke_dialog_ui_labeled_label' + g + '" ', ' id="' + c.labelId + '"', c.inputId ? ' for="' + c.inputId + '"' : "", (a.labelStyle ? ' style="' + a.labelStyle + '"' : "") + ">", a.label, "</label>", '<div class="cke_dialog_ui_labeled_content"',
        a.controlStyle ? ' style="' + a.controlStyle + '"' : "", ' role="presentation">', f.call(this, b, a), "</div>") : (g = {
        type: "hbox",
        widths: a.widths,
        padding: 0,
        children: [{
         ...
         type: "html",
         html: '<span class="cke_dialog_ui_labeled_content"' + (a.controlStyle ? ' style="' + a.controlStyle +
    ...
    select: function(b, a, d) {
     if (!(3 > arguments.length)) {
      ...
      CKEDITOR.ui.dialog.labeledElement.call(this, b, a, d, function() {
       ...
       e.push('<div class="cke_dialog_ui_input_',
   ...
   CKEDITOR.ui.dialog.file.prototype = CKEDITOR.tools.extend(new CKEDITOR.ui.dialog.labeledElement, q, {
    ...
    reset: function() {
     function b() {
      ...
      d.$.write(['<html dir="' + g + '" lang="' + k + '"><head><title></title></head><body style="margin: 0; overflow: hidden; background: transparent;">', '<form enctype="multipart/form-data" method="POST" dir="' + g + '" lang="' + k + '" action="', CKEDITOR.tools.htmlEncode(f.action), '"><label id="', a.labelId, '" for="', h, '" style="display:none">', CKEDITOR.tools.htmlEncode(f.label),
       '</label><input style="width:100%" id="', h, '" aria-labelledby="', a.labelId, '" type="file" name="', CKEDITOR.tools.htmlEncode(f.id || "cke_upload"), '" size="', CKEDITOR.tools.htmlEncode(0 < b ? b : ""), '" /></form></body></html><script>', CKEDITOR.env.ie ? "(" + CKEDITOR.tools.fixDomain + ")();" : "", "window.parent.CKEDITOR.tools.callFunction(" + e + ");", "window.onbeforeunload = function() {window.parent.CKEDITOR.tools.callFunction(" + m + ")}", "</script>"
      ].join(""));
      d.$.close();
   ...
   if (e != CKEDITOR.DIALOG_RESIZE_NONE) {
    ...
    a.on("load", function() {
     ...
     b = CKEDITOR.dom.element.createFromHtml('<div class="cke_resizer' +
      b + " cke_resizer_" + f.lang.dir + '" title="' + CKEDITOR.tools.htmlEncode(f.lang.common.resize) + '" onmousedown="CKEDITOR.tools.callFunction(' + n + ', event )">' + ("ltr" == f.lang.dir ? "◢" : "◣") + "</div>");
  ...
  CKEDITOR.dialog.prototype = {
   ...
   addPage: function(a) {
    if (!a.requiredContent || this._.editor.filter.check(a.requiredContent)) {
     ...
     c = CKEDITOR.dom.element.createFromHtml(['<a class="cke_dialog_tab"', 0 < this._.pageCount ? " cke_last" : "cke_first", c, a.hidden ? ' style="display:none"' : "", ' id="', e, '"', d.gecko && !d.hc ? "" : ' href="javascript:void(0)"',
 ...
 (function() {
  ...
  function B(b, a) {
   ...
   CKEDITOR.env.webkit && -1 < a.indexOf("<div>") && (a = a.replace(/^(<div>(<br>|)<\/div>)(?!$|(<div>(<br>|)<\/div>))/g, "<br>").replace(/^(<div>(<br>|)<\/div>){2}(?!$)/g, "<div></div>"), a.match(/<div>(<br>|)<\/div>/) && (a = "<p>" + a.replace(/(<div>(<br>|)<\/div>)+/g, function(a) {
    return c(a.split("</div><div>").length +
     1)
   ...
   proto: {
    ...
    show: function(a, b, c, m) {
     ...
     for (var d = l.elementPath(), d = ['<div class="cke_menu' + (d && d.direction() != l.lang.dir ? " cke_mixed_dir_content" : "") + '" role="presentation">'], h = e.length, p = h && e[0].group, n = 0; n < h; n++) {
      var q = e[n];
      p != q.group && (d.push('<div class="cke_menuseparator" role="separator"></div>'),
       p = q.group);
      q.render(this, n, d)
     }
     d.push("</div>");
 ...
 (function() {
  function q(a, d) {
   ...
   var p = a.ui.spaceId("path"),
    ...
    q = k.idBase;
   d.html += '<span id="' + p + '_label" class="cke_voice_label">' + a.lang.elementspath.eleLabel + '</span><span id="' +
    p + '" class="cke_path" role="group" aria-labelledby="' + p + '_label"><span class="cke_path_empty">&nbsp;</span></span>';
   a.on("uiReady", function() {
   ...
   proto: {
    setData: function(a, f) {
     ...
     if (f) this.setHtml(a), this.fixInitialSelection(), b.fire("dataReady");
      ...
      d ? (/<body[\s|>]/.test(a) || (a = "<body>" + a), /<html[\s|>]/.test(a) || (a = "<html>" + a + "</html>"), /<head[\s|>]/.test(a) ? /<title[\s|>]/.test(a) || (a = a.replace(/<head[^>]*>/, "$&<title></title>")) : a = a.replace(/<html[^>]*>/, "$&<head><title></title></head>"), h && (a = a.replace(/<head[^>]*?>/, "$&" + h)), a = a.replace(/<\/head\s*>/, g + "$&"), a =
       e + a) : a = c.docType + '<html dir="' + c.contentsLangDirection + '" lang="' + (c.contentsLanguage || b.langCode) + '"><head><title>' + this._.docTitle + "</title>" + h + g + "</head><body" + (c.bodyId ? ' id="' + c.bodyId + '"' : "") + (c.bodyClass ? ' class="' + c.bodyClass + '"' : "") + ">" + a + "</body></html>";
